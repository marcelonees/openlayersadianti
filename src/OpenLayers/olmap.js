...
(function () {
    ...
    var GeoMapApp = (function () {
        ...
        /* ======================================== */
        /* NOVAS VARIÁVEIS PARA CONTROLE DE HIGHLIGHT */
        /* ======================================== */
        ...
        function _setHighlightEnabled(enabled) {
            ...
            /* Atualiza o estado do FeatureInspectControl */
        ...
        function _updateHighlightFeatures() {
            /* Se o highlight estiver desabilitado, limpa e retorna */
            if (!_highlightConfig.enabled) {
                if (_highlightSource) {
                    _highlightSource.clear();
                    _highlightActive = false;
        ...
        function _fetchWfsFeatures() {
            /* VERIFICAÇÃO CRÍTICA - Só executa se o highlight estiver habilitado */
            if (!_highlightConfig.enabled) {
                console.log(
                    "🎯 Highlight desabilitado, ignorando requisição WFS",
                );
                return;
            }
            ...
            if (currentZoom < _highlightConfig.minZoom) {
                console.log(
                    "🎯 Zoom abaixo do mínimo, ignorando requisição WFS",
                );
                return;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    /* VERIFICAÇÃO CRÍTICA - Só processa se o highlight ainda estiver habilitado */
                    if (!_highlightConfig.enabled) {
                        console.log(
        ...
        function _initMap(config) {
            ...
            /* Configuração de highlight - DEVE SER A PRIMEIRA COISA */
            if (config.highlight) {
            ...
            try {
                ...
                _initHighlightLayer();
                _initPopup();
                _initControls(); /* Agora respeita _highlightConfig.enabled */
                ...
                if (config.target) {
                    ...
                    console.log(
                        `✅ Instância do mapa registrada: ${config.target}`,
                    );
                }
                ...
                /* Inicializa o highlight SOMENTE se estiver habilitado */
                if (_highlightConfig.enabled) {
                    setTimeout(function () {
                        _updateHighlightFeatures();
                    }, 500);
                    ...
                    console.log(
                        "🎯 Highlight desabilitado na inicialização - NENHUMA requisição WFS será feita",
                    );
                    /* Garante que o highlightSource esteja vazio */
                    if (_highlightSource) {
                        _highlightSource.clear();
                        _highlightActive = false;
                    }
        ...
        function _addLayerInternal(name, config) {
            ...
            if (!_map) {
                console.warn(
                    "⚠️ Mapa não inicializado, adicionando à fila:",
                    name,
                );
            ...
            try {
                ...
                if (layer) {
                    ...
                    console.log(
                        `✅ Camada adicionada: ${name} (${layerConfig.title})`,
                    );
        ...
        function _handleMapMoveEnd() {
            _updateConfigField();
            /* Atualiza o highlight SOMENTE se estiver habilitado */
            if (_highlightConfig.enabled) {
                _updateHighlightFeatures();
            } else if (_highlightSource) {
                /* Se estiver desabilitado, garante que o source está limpo */
                _highlightSource.clear();
                _highlightActive = false;
            }
        }
        ...
        function _restoreMapConfig(configData) {
            ...
            try {
                ...
                if (config.layers) {
                    Object.keys(config.layers).forEach(function (name) {
                        ...
                        if (layer) {
                            ...
                            const checkbox = _getLastElement(
                                "#layer_chk_" + name,
                            );
                            if (checkbox) {
                ...
                if (
                    config.ui &&
                    config.ui.layerControlCollapsed !== undefined
                ) {
            ...
            _map.getLayers()
                ...
                .forEach(function (layer) {
                    ...
                    checkbox.onchange = function (e) {
                        ...
                        const opacitySlider = _getLastElement(
                            "#layer_opacity_" + name,
                        );
                        if (opacitySlider) {
        ...
        function _initControls() {
            ...
            try {
                ...
                /* SÓ CRIA O FEATURE INSPECT SE O HIGHLIGHT ESTIVER HABILITADO */
                if (_highlightConfig.enabled) {
                    console.log(
                    ...
                    if (_map && _controls.featureInspect) {
                        setTimeout(() => {
                            try {
                                ...
                                console.error(
                                    "Error enabling feature inspect:",
                                    e,
                                );
                            }
                        }, 300);
                    ...
                    /* Cria um placeholder vazio para evitar erros */
        ...
        function _updateCoordinateFields(coordinate) {
            ...
            const latField =
                allLatFields.length > 0
                    ? allLatFields[allLatFields.length - 1]
                    : null;
            ...
            const lonField =
                allLonFields.length > 0
                    ? allLonFields[allLonFields.length - 1]
                    : null;
            ...
            if (latField) {
                ...
                setTimeout(
                    () => latField.classList.remove("coordinate-updated"),
                    500,
                );
            } else {
            ...
            if (lonField) {
                ...
                setTimeout(
                    () => lonField.classList.remove("coordinate-updated"),
                    500,
                );
            } else {
        ...
        class FeatureInspectControl extends ol.control.Control {
            ...
            _onDragUpdate() {
                ...
                /* O highlight agora é gerenciado pelo sistema de highlight configurável */
                _updateHighlightFeatures();
            }
    ...
    window.HighlightAndFlyToGeom = function (geom, zoom) {
        ...
        if (!map) {
            console.warn(
                "⚠️ Nenhum mapa disponível para HighlightAndFlyToGeom",
            );
            return;
        }
        ...
        try {
            ...
            console.log(
                `✅ Voo para geometria no último mapa (zoom: ${zoomLevel})`,
            );
        } catch (e) {
    ...
    /* NOVAS FUNÇÕES GLOBAIS PARA CONTROLE DE HIGHLIGHT */
